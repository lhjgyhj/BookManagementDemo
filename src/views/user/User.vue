...
<el-input v-model="params.name" style="width: 240px" placeholder="请输入名称"></el-input>
...
<el-table-column prop="sex" label="性别"></el-table-column>
  ...
  <el-button  type="warning" @click="handleAccountAdd(scope.row)">充值</el-button>
    <el-button  type="primary" @click="$router.push('/user/edit?id=' + scope.row.id)">编辑</el-button>
    <el-popconfirm
...
export default {
  ...
  methods: {
    changeStatus(row){
      request.put/*后端如果是postMapping则用post,PutMapping则用put*/('user/update',row).then(res=>{
        if (res.code==='200') {
