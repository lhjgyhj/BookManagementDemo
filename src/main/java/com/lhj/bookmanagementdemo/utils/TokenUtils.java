package com.lhj.bookmanagementdemo.utils;
...
public class TokenUtils {
    ...
    public static String genToken(String adminId, String sign,int days) {
        return JWT.create().withAudience(adminId) // 将 admin id 保存到 token 里面,作为载荷
                .withExpiresAt(DateUtil.offsetDay(new Date(), days)) // ...后token过期
    ...
    public static Admin getCurrentAdmin() {
        ...
        try {
            ...
            token = request.getHeader("token");//获取token
            ...
            String adminId = JWT.decode(token)/*解开token*/.getAudience().get(0);
